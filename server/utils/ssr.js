...
export { getInitialData } from 'server/utils/initData';
export { slugifyString, generateHash } from 'utils/strings';
...
export const generateMetaComponents = ({
	...
	let hostname = `https://${initialData.locationData.hostname}`;
	if (hostname.indexOf('localhost') > -1) {
		hostname = 'http://localhost:9876';
	}
	const siteName = 'R1';
	const url = `${hostname}${initialData.locationData.path}`;
	const favicon = `${hostname}/favicon.png`;
